...
router.post('/refresh', (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ id: decoded.id, email: decoded.email }, SECRET, { expiresIn: '15m' });
    const refreshToken = jwt.sign({ id: decoded.id, email: decoded.email }, REFRESH_SECRET, { expiresIn: '7d' });
    validRefreshTokens.add(refreshToken);
    res.json({ accessToken, refreshToken });
  } catch {
    return res.status(401).json({ message: 'Invalid refresh token' });
